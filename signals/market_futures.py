...
def _load_cache() -> dict:
    """Load cache, clearing if it's from a previous calendar day."""
    try:
        with open(CACHE_FILE) as f:
            cache = json.load(f)
        # Invalidate entire cache if it's from a previous day
        today = datetime.utcnow().strftime("%Y-%m-%d")
        for key in ("_fetched_at_premarket", "_fetched_at_close"):
            ts = cache.get(key, "")
            if ts and ts[:10] != today:
                logger.debug(f"futures_cache: stale (from {ts[:10]}) — clearing")
                return {}
        return cache
    except Exception:
        return {}
...
def _fetch_one(ticker: str, mode: str = "premarket") -> tuple[float | None, float | None]:
    ...
    try:
        ...
        # Don't trust fast_info for prev_close — it returns stale values.
        # Always compute from history for accuracy.
        prev = None
        try:
            hist_prev = t.history(period="5d", interval="1d")
            if hist_prev is not None and len(hist_prev) >= 2:
